...
/** Upsert demo panel surveys (same definitions as `npm run seed:panel-surveys`). */
export async function executePanelSurveySeed(): Promise<PanelSurveySeedResult> {
  let inserted = 0;
  let updated = 0;
  let skipped = 0;
  ...
  for (const row of SEED_SURVEYS) {
    ...
    if (!provider) {
      const msg = `Skipped ${row.surveyCode}: provider ${row.providerCompanyCode} not found (run seed:survey-providers first).`;
      warnings.push(msg);
      skipped += 1;
      continue;
    }
  ...
  return {
    ...
    updated,
    skipped,
    definitions: SEED_SURVEYS.length,
    warnings
  };
}
